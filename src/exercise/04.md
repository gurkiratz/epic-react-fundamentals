...
- Fundamentally, JSX just provides syntactic sugar for the
  `React.createElement(component, props, ...children)` function. More 👉
  [https://reactjs.org/docs/jsx-in-depth.html](https://reactjs.org/docs/jsx-in-depth.html)
- Remember that it's Babel that's responsible for taking our JSX and compiling
  it to `React.createElement` calls
- Going deep, the component we try to create is likewise in the language of
  React API, `React.createElement(function)` can actually recieve a function
  name which eventually returns a div. So, we name this function as 'component'.
  `function message(props) { return <div className="message" {...props} />; } `
- Extra Credit 4 explains concept of propTypes for validation. To run
  typechecking on the props for a component, you can assign the special
  propTypes property.
  [https://reactjs.org/docs/typechecking-with-proptypes.html](https://reactjs.org/docs/typechecking-with-proptypes.html)
- _Note: When using JavaScript extensions like Flow or TypeScript, you don't
  need to use propTypes._
- To have side by side elements in JSX, we can use `<> </>` to represent it. It
  is same as writing `<React.Fragment> </React.Fragment>`
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=04%3A%20Creating%20custom%20components&em=singh.gurkirat2707%40gmail.com).
