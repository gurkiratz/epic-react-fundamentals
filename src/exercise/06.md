...
- General DOM API onSubmit refreshes the page and if provided, takes you to next
  page & also we're making a single page application. That's why use
  `event.preventDefault()` function to prevent default behaviour.
- event.target refers to the clicked `<form>` element. When you attach an event
  to an HTML Element, it will return the event object when the event is
  triggered. The triggered event could be a click, mouse over, mouse down etc.
  The target property of the event object (e) will give the current HTML Element
  that the event is trigged on
- [**useRef**](https://beta.reactjs.org/reference/react/useRef) is a React Hook
  that lets you reference a value that’s not needed for rendering. By using a
  ref, you ensure that:
  ...
  - Changing a ref does not trigger a re-render, so refs are not appropriate for
    storing information that you want to display on the screen. Use state for
    that instead. Read more about
    [choosing between useRef and useState.](https://beta.reactjs.org/learn/referencing-values-with-refs#differences-between-refs-and-state)

- [**Rules of Hooks**](https://reactjs.org/docs/hooks-rules.html) - Hooks are
  JavaScript functions, but you need to follow two rules when using them.

- Using `useState` hook, we can validate form.

- **💯 Control the input value** Sometimes you have form inputs which you want
  to programmatically control. Maybe you want to set their value explicitly when
  the user clicks a button, or maybe you want to change what the value is as the
  user is typing.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=06%3A%20Forms&em=singh.gurkirat2707%40gmail.com).
