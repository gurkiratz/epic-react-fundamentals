...
- So, first we added React and ReactDOM packages directly from
  [UNPKG](https://unpkg.com/) which basically can provide CDN like script for
  any NPM package.
- The new React v18 no longer supports `ReactDOM.render(element, root)` but
  insted we have to use `createRoot` like this
  `ReactDOM.createRoot(root).render(element)`
- So basically the React API will then be used for writing JSX
- **The extra credit** provided a good look over the children props while adding
  multiple elements inside it (in the form of array).
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=02%3A%20Intro%20to%20raw%20React%20APIs&em=singh.gurkirat2707%40gmail.com).
